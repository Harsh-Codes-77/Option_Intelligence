...
export async function runBreadthEngine(data: BreadthData | null): Promise<BreadthResult> {
  ...
  let data_status: DataStatus = (advancing + declining === 0) ? 'NO_DATA' : 'LIVE';

  if (data_status === 'NO_DATA') {
    return { ...defaults, advancing, declining, unchanged };
  }
  ...
  return {
    ...
    formulaBreakdown: {
      ...
      steps: [
        { step: 0, label: 'Data Status', formula: 'Validating A/D data', value: data_status },
